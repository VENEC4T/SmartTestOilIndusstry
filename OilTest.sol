...
contract TheOilIndustry {
    ...
    function upgradeTower(uint256 floorId) public {
        ...
        totalWorkers++;
        uint256 Workers = towers[user].Workers[floorId];
    ...
    function sellTower() public {
        collectMoney();
        address user = msg.sender;
        uint8[8] memory Workers = towers[user].Workers;
        totalWorkers -= Workers[0] + Workers[1] + Workers[2] + Workers[3] + Workers[4] + Workers[5] + Workers[6] + Workers[7];
